function isMovingAway(axis, particle) {
  const position = particle.position[axis];
  const velocity = particle.velocity[axis];
  const acceleration = particle.acceleration[axis];
...
function collide(particles) {
  ...
  for (let i = 0; i < particles.length; i++) {
    ...
    const key = `${x}:${y}:${z}`;
    if (!particlesAtPosition[key]) {
      particlesAtPosition[key] = [];
    }
    particlesAtPosition[key].push(particle);
  }

  let destroyCount = 0;
  Object.keys(particlesAtPosition).forEach(collisionPosition => {
    if (particlesAtPosition[collisionPosition].length > 1) {
      particlesAtPosition[collisionPosition].forEach(particle => {
...
function processInput(input) {
  const particleStrings = input.split('\n');
  const particles = [];
  let movingTowardsXCountLeft = particleStrings.length;
  let movingTowardsYCountLeft = particleStrings.length;
  let movingTowardsZCountLeft = particleStrings.length;

  for (let i = 0; i < particleStrings.length; i++) {
    const matches = particleStrings[i].match(/^p=<\s*(.*?)\s*>, v=<\s*(.*?)\s*>, a=<\s*(.*?)\s*>$/);
    const position = matches[1].split(',').map(number => parseInt(number));
    const velocity = matches[2].split(',').map(number => parseInt(number));
    const acceleration = matches[3].split(',').map(number => parseInt(number));
  ...
  let particleCountAfterAllCollisions = particles.length;
  ...
  while (true) {
    for (let i = 0; i < particles.length; i++) {
      ...
      if (!closestParticle || closestParticle.distance > particle.distance) {
        closestParticle = particle;
      }

      if (!particle.isMovingAwayX && isMovingAway('x', particle)) {
        movingTowardsXCountLeft--;
        particle.isMovingAwayX = true;
      }
      if (!particle.isMovingAwayY && isMovingAway('y', particle)) {
        movingTowardsYCountLeft--;
        particle.isMovingAwayY = true;
      }
      if (!particle.isMovingAwayZ && isMovingAway('z', particle)) {
        movingTowardsZCountLeft--;
        particle.isMovingAwayZ = true;
      }
    }

    particleCountAfterAllCollisions -= collide(particles);

    if (
      movingTowardsXCountLeft === 0 &&
      movingTowardsYCountLeft === 0 &&
      movingTowardsZCountLeft === 0
    ) {
      break;
    }
  }
  ...
  while (true) {
    ...
    const previousCount = particleCountAfterAllCollisions;
    particleCountAfterAllCollisions -= collide(particles);

    if (particleCountAfterAllCollisions === previousCount) {
      break;
    }
  }
  ...
  return {
    closestParticle,
    particleCountAfterAllCollisions,
  };
}

function part1(input) {
  return processInput(input).closestParticle.index;
}

function part2(input) {
  return processInput(input).particleCountAfterAllCollisions;
}
