function getData(input) {
  ...
  for (const line of lines) {
    ...
    const children = matches[4] ? matches[4].split(', ') : [];
...
function createTree(name, weight, childrenByName, weightsByName) {
  return {
    name,
    weight,
    children: childrenByName[name].map(childName => {
      return createTree(
        childName,
        weightsByName[childName],
        childrenByName,
        weightsByName,
      );
    }),
...
function getTreeWeight(tree) {
  return tree.children.reduce((weight, child) => {
    return weight + getTreeWeight(child);
  }, tree.weight);
}

function findUnbalancedWeight(tree) {
  const weightCounts = {};
  const childIndexByWeight = {};

  for (const child of tree.children) {
    const weight = getTreeWeight(child);
    weightCounts[weight] = (weightCounts[weight] || 0) + 1;
  }

  const expectedWeightIndex = Object.keys(weightCounts).findIndex(weight => {
    return weightCounts[weight] > 1;
  });
  const expectedWeight = parseInt(Object.keys(weightCounts)[expectedWeightIndex], 10);

  if (tree.children.length > 1) {
    for (const weight of Object.keys(weightCounts)) {
      if (weightCounts[weight] === 1) {
        for (const child of tree.children) {
          const childTreeWeight = getTreeWeight(child);
          if (childTreeWeight === parseInt(weight, 10)) {
            return findUnbalancedWeight(child) || {
              item: child,
              treeWeight: childTreeWeight,
              expectedWeight,
            };
          }
        }
...
function part1(input) {
  ...
  for (const {name, children} of getData(input)) {
    names.push(name);
...
function part2(input) {
  ...
  for (const {name, weight, children} of getData(input)) {
    childrenByName[name] = children;
    weightsByName[name] = weight;
  }

  const tree = createTree(
    rootName,
    weightsByName[rootName],
    childrenByName,
    weightsByName,
  );

  const data = findUnbalancedWeight(tree);
  
  if (data) {
    return data.item.weight - (data.treeWeight - data.expectedWeight);
  }
}

console.log('--- Part 1 ---');
console.log('Test:', part1(getTestInput()) === 'tknk' ? 'OK' : 'FAIL');
console.log('Result:', part1(getInput()));

console.log('');
console.log('--- Part 2 ---');
console.log('Test:', part2(getTestInput()) === 60 ? 'OK' : 'FAIL');
