function getInstructionMatcher() {
  const identifier = '([a-z]+)';
  const operation = '(dec|inc)\\s(-?\\d+)';
  const conditionOperator = '(>=|<=|>|<|!=|==)';
  const conditionValue = '(-?\\d+)';
  const condition = (
    `if\\s${identifier}\\s${conditionOperator}\\s${conditionValue}`
  );
  return new RegExp(
    `^${identifier}\\s${operation}\\s${condition}\$`
  );
}
...
function processInput(input) {
  ...
  const registers = {};
  const instructionMatcher = getInstructionMatcher();
  ...
  instructions.forEach(instruction => {
    let [
      ...
      conditionOperator,
      conditionValue,
    ] = instruction.match(instructionMatcher);

    operationValue = parseInt(operationValue, 10);
    conditionValue = parseInt(conditionValue, 10);

    if (!registers.hasOwnProperty(subject)) {
      registers[subject] = 0;
    }

    if (!registers.hasOwnProperty(conditionSubject)) {
      registers[conditionSubject] = 0;
    }

    let passesCondition = false;
    const a = registers[conditionSubject];
    const b = conditionValue;

    switch (conditionOperator) {
      case '>=': passesCondition = a >= b; break;
      case '<=': passesCondition = a <= b; break;
      case '>': passesCondition = a > b; break;
      case '<': passesCondition = a < b; break;
      case '!=': passesCondition = a !== b; break;
    ...
    if (passesCondition) {
      switch (operation) {
        case 'inc': registers[subject] += operationValue; break;
        case 'dec': registers[subject] -= operationValue; break;
      }
    }

    if (registers[subject] > maxEver) {
      maxEver = registers[subject];
    }
  });

  let max = -Infinity;
  Object.values(registers).forEach(value => {
    if (value > max) {
      max = value;
    }
  });
...
function part1(input) {
  return processInput(input).max;
}

function part2(input) {
  return processInput(input).maxEver;
}

console.log('--- Part 1 ---');
console.log('Test:', part1(getTestInput()) === 1 ? 'OK' : 'FAIL');
console.log('Result:', part1(getInput()));

console.log('');
console.log('--- Part 2 ---');
console.log('Test:', part2(getTestInput()) === 10 ? 'OK' : 'FAIL');
